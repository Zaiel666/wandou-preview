...
foreach($entry in $backup){
    if(-not($entry.Path -like 'Software\Classes\*' -or $entry.Path -eq 'Software\WandouPreview' -or $entry.Path -eq 'Software\Microsoft\Windows\CurrentVersion\Run' -or $entry.Path -eq 'Software\Microsoft\Windows\CurrentVersion\Uninstall\WandouPreview')){throw '安装备份包含意外的注册表路径。'}
