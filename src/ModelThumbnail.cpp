...
static void Badge(HBITMAP bitmap,const std::wstring& label){
    ...
    int bitmapHeight=int(info.bmHeight);int h=std::max(13,std::min(26,bitmapHeight/8)),w=std::max(h*2,int(label.size()*h*.62f)),pad=std::max(3,h/4);RECT rect={pad,bitmapHeight-pad-h,pad+w,bitmapHeight-pad};
...
static HRESULT RenderWorker(const std::wstring& file,UINT size,HBITMAP* bitmap){
    HMODULE self=nullptr;GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,reinterpret_cast<LPCWSTR>(&RenderWorker),&self);wchar_t module[32768];if(!GetModuleFileNameW(self,module,32768))return E_FAIL;std::wstring folder(module);folder=folder.substr(0,folder.find_last_of(L"\\/"));std::wstring extension=Extension(file);bool image=extension==L"AI"||extension==L"HDR"||extension==L"BLEND",video=IsVideo(extension);std::wstring exe=folder+(image?L"\\WandouImagePreview.exe":video?L"\\WandouVideoThumbnail.exe":L"\\WandouModelPreview.exe");
    wchar_t tempDir[MAX_PATH],tempFile[MAX_PATH];if(!GetTempPathW(MAX_PATH,tempDir)||!GetTempFileNameW(tempDir,L"wdp",0,tempFile))return E_FAIL;UINT renderSize=std::max(128u,std::min(768u,size));std::wstring imageMode=extension==L"AI"?L"--extract-ai-bmp":extension==L"HDR"?L"--extract-hdr-bmp":L"--extract-blend-bmp";std::wstring command=image?(L"\""+exe+L"\" "+imageMode+L" \""+file+L"\" \""+tempFile+L"\" "+std::to_wstring(renderSize)):video?(L"\""+exe+L"\" \""+file+L"\" \""+tempFile+L"\""):(L"\""+exe+L"\" --render \""+file+L"\" \""+tempFile+L"\" "+std::to_wstring(renderSize));
    STARTUPINFOW startup={sizeof(startup)};PROCESS_INFORMATION process={};HANDLE job=CreateJobObjectW(nullptr,nullptr);if(!job){DeleteFileW(tempFile);return E_FAIL;}JOBOBJECT_EXTENDED_LIMIT_INFORMATION limits={};limits.BasicLimitInformation.LimitFlags=JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE|JOB_OBJECT_LIMIT_PROCESS_MEMORY;limits.ProcessMemoryLimit=(image||video?1536ull:3072ull)*1024*1024;DWORD waitMilliseconds=image?20000:(video?30000:60000);HRESULT hr=E_FAIL;
    if(SetInformationJobObject(job,JobObjectExtendedLimitInformation,&limits,sizeof(limits))&&CreateProcessW(exe.c_str(),&command[0],nullptr,nullptr,FALSE,CREATE_NO_WINDOW|CREATE_SUSPENDED,nullptr,nullptr,&startup,&process)){
        if(AssignProcessToJobObject(job,process.hProcess)){ResumeThread(process.hThread);if(WaitForSingleObject(process.hProcess,waitMilliseconds)==WAIT_OBJECT_0){DWORD code=1;GetExitCodeProcess(process.hProcess,&code);if(!code){*bitmap=(HBITMAP)LoadImageW(nullptr,tempFile,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION);if(*bitmap)hr=S_OK;}}}
