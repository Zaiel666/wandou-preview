...
static void Badge(HBITMAP bitmap,const std::wstring& label){
    ...
    int h=max(13,min(26,info.bmHeight/8)),w=max(h*2,int(label.size()*h*.62f)),pad=max(3,h/4);RECT rect={pad,info.bmHeight-pad-h,pad+w,info.bmHeight-pad};
...
static void Fit(HBITMAP* bitmap,UINT size){
    BITMAP info={};if(!GetObjectW(*bitmap,sizeof(info),&info)||!size)return;int longest=max(info.bmWidth,info.bmHeight);if(longest<=int(size))return;
    int w=max(1,int((long long)info.bmWidth*size/longest)),h=max(1,int((long long)info.bmHeight*size/longest));BITMAPINFO bi={};bi.bmiHeader.biSize=sizeof(BITMAPINFOHEADER);bi.bmiHeader.biWidth=w;bi.bmiHeader.biHeight=-h;bi.bmiHeader.biPlanes=1;bi.bmiHeader.biBitCount=32;bi.bmiHeader.biCompression=BI_RGB;void* bits=nullptr;HBITMAP scaled=CreateDIBSection(nullptr,&bi,DIB_RGB_COLORS,&bits,nullptr,0);if(!scaled)return;
...
static HRESULT RenderWorker(const std::wstring& file,UINT size,HBITMAP* bitmap){
    ...
    wchar_t tempDir[MAX_PATH],tempFile[MAX_PATH];if(!GetTempPathW(MAX_PATH,tempDir)||!GetTempFileNameW(tempDir,L"mqp",0,tempFile))return E_FAIL;std::wstring command=L"\""+exe+L"\" --render \""+file+L"\" \""+tempFile+L"\" "+std::to_wstring(max(128u,min(768u,size)));
