...
static HRESULT RenderWorker(const std::wstring& file,UINT size,HBITMAP* bitmap){
    HMODULE self=nullptr;GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,reinterpret_cast<LPCWSTR>(&RenderWorker),&self);wchar_t module[32768];if(!GetModuleFileNameW(self,module,32768))return E_FAIL;std::wstring folder(module);folder=folder.substr(0,folder.find_last_of(L"\\/"));std::wstring extension=Extension(file);bool image=extension==L"AI"||extension==L"HDR";std::wstring exe=folder+(image?L"\\WandouImagePreview.exe":L"\\WandouModelPreview.exe");
    wchar_t tempDir[MAX_PATH],tempFile[MAX_PATH];if(!GetTempPathW(MAX_PATH,tempDir)||!GetTempFileNameW(tempDir,L"wdp",0,tempFile))return E_FAIL;UINT renderSize=std::max(128u,std::min(768u,size));std::wstring imageMode=extension==L"AI"?L"--extract-ai-bmp":L"--extract-hdr-bmp";std::wstring command=image?(L"\""+exe+L"\" "+imageMode+L" \""+file+L"\" \""+tempFile+L"\" "+std::to_wstring(renderSize)):(L"\""+exe+L"\" --render \""+file+L"\" \""+tempFile+L"\" "+std::to_wstring(renderSize));
