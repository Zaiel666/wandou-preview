...
static HWND MakeWindow(HINSTANCE instance,bool hidden,int size){WNDCLASSEXW wc={sizeof(wc),CS_OWNDC|CS_DBLCLKS,WindowProc,0,0,instance,LoadIconW(instance,MAKEINTRESOURCEW(101)),LoadCursor(nullptr,IDC_ARROW),(HBRUSH)(COLOR_WINDOW+1),nullptr,L"ModelQuickPreview",LoadIconW(instance,MAKEINTRESOURCEW(101))};RegisterClassExW(&wc);DWORD style=hidden?WS_POPUP:WS_OVERLAPPEDWINDOW;return CreateWindowExW(0,wc.lpszClassName,(L"3D 模型快速预览 · "+BaseName(g.file)).c_str(),style,CW_USEDEFAULT,CW_USEDEFAULT,size,size,nullptr,nullptr,instance,nullptr);}
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR,int){
    int count=0;LPWSTR* args=CommandLineToArgvW(GetCommandLineW(),&count);if(count<2){MessageBoxW(nullptr,L"请右键一个支持的三维模型，选择“3D 模型快速预览”。",L"3D 模型快速预览",MB_ICONINFORMATION);return 0;}
    ...
    if(!LoadModel(g.file)){std::wstring detail=L"无法读取这个模型。\n\n"+std::wstring(g.error.begin(),g.error.end());if(!render)MessageBoxW(nullptr,detail.c_str(),L"3D 模型快速预览",MB_ICONERROR);LocalFree(args);return 2;}
