...
static class Setup {
    [STAThread] static int Main(string[] args) {
        ...
        string log=Path.Combine(Path.GetTempPath(),"WandouPreview-Setup.log");
        try {
            ...
            var start=new ProcessStartInfo(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Windows),"System32","WindowsPowerShell","v1.0","powershell.exe"),"-NoProfile -ExecutionPolicy Bypass -File \""+script+"\""){UseShellExecute=false,CreateNoWindow=true,WorkingDirectory=temporary,RedirectStandardOutput=true,RedirectStandardError=true};
            using(Process process=Process.Start(start)){string output=process.StandardOutput.ReadToEnd();string error=process.StandardError.ReadToEnd();if(!process.WaitForExit(90000)){try{process.Kill();}catch{}throw new Exception("安装超过 90 秒，已停止。");}if(process.ExitCode!=0)throw new Exception("安装脚本返回错误代码 "+process.ExitCode+"。\r\n"+error+output);}
        ...
        } catch(Exception error){try{File.WriteAllText(log,error.ToString());}catch{}if(!quiet)MessageBox.Show("安装失败："+error.Message+"\n\n错误日志："+log,"豌豆预览",MessageBoxButtons.OK,MessageBoxIcon.Error);return 1;}
